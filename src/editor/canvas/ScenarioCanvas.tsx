...
function BoatGraphic({
  ...
  const lacustreCloseHauled =
    ...
    isCloseHauled(object.heading, windDirection, laylineAngle)
  ...
  const sailAngle = constrainSailAngle(
    object.sailMode === 'automatic'
      ? automaticMain + object.mainsailTrim
      : object.sailAngle,
    object.heading,
    windDirection,
  )
    ...
    >
      ...
      <Group scaleX={profile.displayScale} scaleY={profile.displayScale}>
        <Path
          ...
          stroke={profile.kind === 'vsr' ? 'transparent' : '#171717'}
              ...
              data={
                spinnakerStalled
                  ? luffingSpinnakerPath(
                      ...
                      profile.length / 2 - profile.mast[1] + profile.spinnakerSize * 0.18,
                    )
...
function MarkGraphic({ object, selected, onSelect, onChange, zoneBoatLength }: ObjectGraphicProps) {
  ...
  const labelFontSize = markLabel.length <= 2 ? 14 : markLabel.length <= 4 ? 10 : 8
...
export const ScenarioCanvas = forwardRef<CanvasHandle>(function ScenarioCanvas(_, ref) {
  ...
  useEffect(() => {
    ...
    const measurementHidden = nodes.flatMap((node) => {
      if (node.getType() !== 'Group') return []
      const group = node as Konva.Group
      const rotationBounds = group.findOne('.rotation-bounds')
      if (!rotationBounds) return group.find('.selection-bounds-ignore')

      const keepVisible = new Set<Konva.Node>([node, rotationBounds])
      let ancestor = rotationBounds.getParent()
      while (ancestor && ancestor !== node) {
        keepVisible.add(ancestor)
        ancestor = ancestor.getParent()
      }
      return group.find(
        (candidate: Konva.Node) => !keepVisible.has(candidate) && candidate.visible(),
      )
    })
    measurementHidden.forEach((node) => node.hide())
    transformer.nodes(nodes)
    transformer.forceUpdate()
    measurementHidden.forEach((node) => node.show())
  ...
  return (
    <div ref={containerRef} className="editor-canvas" aria-label={t('Sailing scenario canvas')}>
      <Stage
        ref={stageRef}
        width={size.width}
        height={size.height}
