...
const profiles = {
  ...
  vsr: {
    hullPath:
      ...
      'L 13 24 ' +
      'C 13 32 11.5 37 8.5 39 ' +
      'L 5.5 40 ' +
      'L -5.5 40 ' +
      'L -8.5 39 ' +
      'C -11.5 37 -13 32 -13 24 ' +
      'L -12 -14 ' +
...
export function upwindSailVisibility(boatClass: BoatClass) {
  ...
  return {
    ...
    jibVisible: Boolean(profile.jibTack && profile.jibSize),
...
export function automaticSpinnakerAngle(heading: number, windDirection: number): number {
  const relative = relativeWindAngle(heading, windDirection)
  if (relative < 80) return relative
  if (relative > 280) return relative - 360
  return relative < 180 ? relative - 20 : relative + 20
}
...
export function spinnakerPath(size: number, side: -1 | 1, stalled: boolean): string {
  if (stalled) {
    return `${curvedSailPath(size, side, true).replace(/ Z$/, '')} C ${side * 0.08 * size} ${0.8 * size} ${side * 0.08 * size} ${0.4 * size} 0 0 Z`
  }
